app.controller('CtrlFilter', ['$scope','ajax','$rootScope','$filter', function(scope,ajax,root,$filter) {
	...
	scope.avancetotal = function (curr_total, old_total) {
		...
		// console.info('Reporting avancetotal:', curr_total, old_total);
		// return 0;
	...
	scope.graph = function(object){
		...
		var line = {
			color: 'rgba(141,42,144,0.5)',
			color_two: 'rgba(141,42,144,0.2)',
			labels: ['mes','mes','mes','mes','mes','mes','mes','mes'],
			data: [15,20,30,40,50,60,70,200],
			realData: [1200,1200,1200,1200,1850,1950,1900,1900]
		}

		var line2 = {
			color: 'rgba(141,42,144,0.5)',
			color_two: 'rgba(141,42,144,0.5)',
			labels: ['','','','','','','',''],
			data: [8,9,10,11,22,4,6,30],
			realData: [5,2,4,5,10,20,30,15]
		}

		var ve = new VeGraph('nufarm');
		ve.setPercentages( Math.max.apply(Math, line.data));
		ve.setDataLine(line);
		ve.drawGraphLine();

		ve.setDataLine(line2);
		ve.drawGraphLine();
